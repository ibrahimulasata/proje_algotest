...
async function getUserById(req, res) {
  try {
    const { id } = req.params;
    const q = `SELECT id, fullname, email, created_at FROM users WHERE id = $1`;
    ...
    const user = rows[0];
    if (!isAdmin && !isSelf) delete user.email;
...
async function updateUser(req, res) {
  try {
    ...
    if (value.fullname) { fields.push(`fullname = $${i++}`); params.push(String(value.fullname).trim()); }
    if (value.email)    { fields.push(`email = $${i++}`);    params.push(String(value.email).trim().toLowerCase()); }
    if (value.password) { fields.push(`password_hash = $${i++}`); params.push(await bcrypt.hash(String(value.password), 12)); }

    params.push(id);

    const q = `UPDATE users SET ${fields.join(", ")} WHERE id = $${i} RETURNING id, fullname, email, created_at`;
